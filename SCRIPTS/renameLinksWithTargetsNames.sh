...
##
## $pathRoot          : path of directory where renaming links with their targets names
...
function Syntaxe
{
    echo "This script rename links under pathRoot with their targets names"
    echo "-----------------------------------------------------------------------"
    echo "USAGE : $0 [pathRoot] [depth]*"
    echo "-----------------------------------------------------------------------"
    echo "WHERE :"
    echo "- pathRoot          : path of directory where renaming links with their targets name"
...
if [ $cr -eq 0 ]; then
    ...
    echo "ERROR invalid full PathSrc computed : ("$pathRoot") !"
    exit 99
fi
